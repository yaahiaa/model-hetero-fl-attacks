...
cfg.setdefault('ipfs_gateway_url', 'http://127.0.0.1:8080/ipfs')
...
def write_overhead_summary(row):
    fieldnames = [
        ...
        'mean_artifact_verify_time_sec', 'mean_ledger_submit_candidate_time_sec',
        'mean_ledger_submit_report_time_sec', 'mean_ledger_finalize_time_sec',
        'mean_blockchain_receipt_wait_time_sec', 'total_blockchain_gas_used',
        'mean_ipfs_add_time_sec', 'mean_ipfs_cat_time_sec', 'mean_ipfs_pin_time_sec',
        'relative_notes',
    ]
...
def create_commitment_backend():
    ...
    if artifact_backend not in {'local', 'ipfs'}:
        raise ValueError(f'Unsupported artifact_store_backend: {artifact_backend}')
    if ledger_backend not in {'json', 'ethereum'}:
        raise ValueError(f'Unsupported ledger_backend: {ledger_backend}')

    print('[COMMIT] selected commitment backend:', commitment_backend)
    print('[COMMIT] selected artifact backend:', artifact_backend)
    print('[COMMIT] selected ledger backend:', ledger_backend)
    if artifact_backend == 'ipfs':
        print('[COMMIT] IPFS API URL:', cfg.get('ipfs_api_url'))
    if ledger_backend == 'ethereum':
        print('[COMMIT] blockchain RPC URL:', cfg.get('blockchain_rpc_url'))
        print('[COMMIT] blockchain chain id:', cfg.get('blockchain_chain_id'))
        print('[COMMIT] blockchain contract address:', cfg.get('blockchain_contract_address') or '<deploy>')
    ...
    if artifact_backend == 'local':
        ...
        artifact_store = IPFSArtifactStore(
            ...
            ipfs_gateway_url=cfg.get('ipfs_gateway_url', 'http://127.0.0.1:8080/ipfs'),
    ...
    if ledger_backend == 'json':
        ...
        print('[COMMIT] resolved contract address:', ledger.contract_address)
...
def runExperiment():
    ...
    commitment_overhead_summary = summarize_commitment_overhead(cfg.get('round_log_dir') or os.path.join('output', 'round_log', 'prototype2'))
