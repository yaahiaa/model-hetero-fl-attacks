...
contract CommitmentLedger {
    ...
    event GenesisCommitted(bytes32 parentHash, string cid, bytes32 artifactSha256, uint256 artifactSizeBytes, bytes32 metadataHash, address submitter, uint256 timestamp);
    ...
    function commitGenesisParent(bytes32 parentHash, string calldata cid, bytes32 artifactSha256, uint256 artifactSizeBytes, bytes32 metadataHash) external {
    ...
    function submitVerificationReport(
        ...
        bytes32 attestationHash = keccak256(abi.encode(roundId, candidateParentHash, verifierUserId, cohortRateScaled, approved, reasonCode, failedChecksBitmask, relativeChangeScaled));
        ...
        emit VerificationReportSubmitted(roundId, candidateParentHash, verifierUserId, cohortRateScaled, approved, reasonCode, failedChecksBitmask, relativeChangeScaled, msg.sender);
    }
    ...
    function getLatestApprovedParent() external view returns (bool exists, bytes32 parentHash, string memory cid, bytes32 artifactSha256, uint256 artifactSizeBytes) {
        return (hasLatestApprovedParent, latestApprovedParentHash, latestApprovedParentCid, latestApprovedArtifactSha256, latestApprovedArtifactSizeBytes);
    }
}
